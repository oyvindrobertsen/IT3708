...
\begin{multicols}{2} % Two-column layout throughout the main article text
  ...
  Through several evolutionary runs, with varying parameters used for the runs (increasing selection pressure), the best individuals using this design achieved fitness values between $0.45$ and $0.7$.
  Note that the codomain of our fitness function is $[-1, 1]$.
  While this is not horrible performance, we noticed that these agents decreased their score by making stupid choices, not by being placed in situations in which they would have no other choice than to eat poison.
